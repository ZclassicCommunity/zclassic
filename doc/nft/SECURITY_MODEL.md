...
Status: NORMATIVE. Spec version `ZSLP_SPEC_VERSION = 2` (see §8).
...
| T12 | **Set/collection spoof**: children claiming a set's name with no cryptographic tie. | Set membership by name is unauthenticated. | **RESOLVED (spec-v2, R-NFT1):** closed/authorized membership. A child GENESIS carries an optional 32-byte `group_id` field AND must spend a live parent-token UTXO/baton of that group as an authorization input. The overlay records `group_authorized` only when both hold; the `'g'` membership index + `zslp_listcollectionmembers` return ONLY authorized children. A non-owner can NAME any group (recorded `group_claimed`/`group_authorized=false`) but can never forge membership — authorization reuses consensus' own scriptSig check (R-SEND-2). `claimed` is NEVER surfaced as membership. **Burn-scope note (wallet, not consensus):** the overlay counts the SPENT OUTPOINT as exactly one authorization regardless of the units it holds, and a child is a GENESIS so it cannot return parent-token change — the whole outpoint is burned. The wallet builder MUST therefore authorize each child with a **single-unit** (`amount==1`) parent output, never a multi-unit one (which would silently burn all its units for one card) — see R-NFT1. | Residual: `claimed` (named-but-unauthorized) is representable (same class as ticker squat T11) and MUST render as NOT-a-member; membership is monotonic (no revocation). | INFO (resolved for the authorized set) |
...
- **Set membership** (spec-v2): a token is a verified collection member ONLY when `group_authorized` is true (it named the group AND its GENESIS spent a live parent-token authority input). The GUI surfaces membership EXCLUSIVELY from the `'g'` index / `zslp_listcollectionmembers` (authorized children). A token's `group_claimed` flag (it merely named a group) is NEVER shown as membership — exactly the T11 name-squat reduction. The collection's identity is the parent `groupId` (genesis txid), never its name.
...
- [x] **R-NFT1** (spec-v2) Group/child collection membership rule — IMPLEMENTED + tested. A child GENESIS carries an optional trailing 32-byte `group_id` push (field 10; on-chain/display order, reversed to the daemon uint256 like token_id). A child is an AUTHORIZED member of collection G iff ALL hold, from confirmed chain only: (1) its vout[0] is a valid GENESIS whose `group_id == G`; (2) `readToken(G)` hits (G's GENESIS exists); (3) the child tx spent a live parent-token UTXO OR baton of G on an input (`availByToken.count(G) || batonInputPresent.count(G)`, computed by the same consume pass the MINT baton check uses). The authority outpoint is BURNED IN FULL by the consume step (consume-before-create; one outpoint authorizes exactly one child — no replay; a GENESIS does not conserve unrelated token inputs, so the spent parent outpoint is the authorization and CANNOT carry parent-token change). **Burn-scope (wallet correctness, not consensus):** because the entire spent outpoint is consumed, the WALLET builder (`rpc/zslp.cpp` `zslp_genesis` group path) MUST authorize each child with a parent output holding EXACTLY ONE unit (`amount==1`; collection authority has decimals 0). It refuses to auto-select or pin a multi-unit output (it would silently burn ALL its units for a single card) and tells the user to split the authority into single units first (`zslp_send <G>` to self, per-recipient amount 1, up to 19/tx). The mint baton is spendable as authority only with `allow_baton`, and burning it in a child GENESIS SEALS the collection. None of this changes the overlay forgery rule (authorized iff a real live parent unit/baton was spent); it only prevents the holder's own wallet from over-burning. `groupAuthorized = parentKnown && authIn` rides the existing `UNDO_TOKEN_PUT`; the paired `'g'` membership row rides a new `UNDO_GROUP_INDEX_PUT`, so reorg is byte-exact. The two tiers are `claimed` (group named) vs `authorized` (all three hold); `claimed` is NEVER surfaced as membership. *Determinism tests:* `test_zslp_collections.cpp` (authorized/claimed/nonexistent-group/no-replay/two-indexer-agreement/reorg/legacy-byte-identical/32-byte-gate/intra-block). Closed (issuer-authorized) membership; revocation is not expressible (out of scope).
...
- This document is the NORMATIVE spec at **`ZSLP_SPEC_VERSION = 2`**. It is published (committed in-tree under `doc/nft/`) so any third party can implement `F` and prove agreement. **Spec-v2 adds the group/child collections feature (R-NFT1):** a child GENESIS carries an optional trailing 32-byte `group_id` push. This is a GROUP-FEATURE-ONLY hard fork — a strict v1 parser rejects a child GENESIS (trailing push) as not-SLP, so grouped children are invisible to un-upgraded indexers; **ungrouped tokens and the collection parent itself are unaffected and remain cross-compatible with v1 parsers.** The on-disk `ZSLP_INDEX_VERSION` moves 2 → 3 in lockstep, forcing a clean wipe + reindex so the new group columns + `'g'` index populate deterministically (no v2 record is read with the new schema).
