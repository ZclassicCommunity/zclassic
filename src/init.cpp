...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // ********************************************************* Step 10b: initialize snapshot system

    // Initialize rate limiter for serving (if we have chunks)
    if (!psnapshotratelimiter) {
        psnapshotratelimiter = new CSnapshotRateLimiter();

        // Load config settings
        if (mapArgs.count("-snapshotmaxchunkspermin")) {
            uint32_t maxChunks = GetArg("-snapshotmaxchunkspermin", 30);
            uint32_t maxConcurrent = GetArg("-snapshotmaxconcurrent", 25);
            uint32_t minSecBetween = GetArg("-snapshotminsecbetween", 2);
            psnapshotratelimiter->SetLimits(maxChunks, maxConcurrent, minSecBetween);
        }
    }

    // Initialize snapshot store for serving
    if (!psnapshotstore) {
        psnapshotstore = new CSnapshotStore();
        if (psnapshotstore->Initialize(SNAPSHOT_CURRENT_HEIGHT)) {
            LogPrintf("Snapshot store initialized for height %d\n", SNAPSHOT_CURRENT_HEIGHT);

            // Advertise NODE_SNAPSHOT if we can serve
            if (CanServeSnapshots()) {
                nLocalServices |= NODE_SNAPSHOT;
                LogPrintf("Advertising NODE_SNAPSHOT service (have chunks available)\n");
            }
        }
    }
    ...
    if (needsBlockchain && psnapshotstore) {
        ...
        uint32_t chunkCount = psnapshotstore->GetManifest().GetChunkCount();
        ...
        if (allChunksPresent) {
            LogPrintf("All %d snapshot chunks present - extracting snapshot...\n", chunkCount);

            // Extract snapshot to data directory
            if (psnapshotstore->ExtractSnapshot(GetDataDir())) {
                LogPrintf("Snapshot extracted successfully!\n");

                // IMPORTANT: After extraction, node must restart to load the blockchain
                // The extracted chainstate and blocks will be picked up on next startup
                LogPrintf("*** SNAPSHOT EXTRACTION COMPLETE ***\n");
                LogPrintf("*** Please restart zcl assicd to use the snapshot ***\n");

                return true;  // Exit cleanly - user must restart
            } else {
                LogPrintf("ERROR: Snapshot extraction failed! Falling back to P2P download mode.\n");
                // Continue to download mode below
            }
        } else {
            LogPrintf("Blockchain data insufficient (height=%d), activating P2P snapshot download mode\n", chainActive.Height());

            // Initialize download state
            pdownloadstate = new CSnapshotDownloadState(psnapshotstore->GetManifest().GetChunkCount());
            psnapshotcoordinator = new CSnapshotDownloadCoordinator(pdownloadstate);

            // Enable download mode
            fSnapshotDownloadActive = true;
            LogPrintf("P2P snapshot download mode activated - will download %d chunks from peers\n",
                      psnapshotstore->GetManifest().GetChunkCount());
        }
    }
